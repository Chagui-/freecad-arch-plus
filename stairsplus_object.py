...
# *   Copyright (c) 2026 Andres <andres@neltu.me>                           *
# *                                                                         *
# *   This file is a modified copy of FreeCAD's ArchStairs.py, adapted for  *
# *   the StairsPlus add-on. As a derivative of LGPL-2.1-or-later code it   *
# *   remains licensed under the same terms.                                *
# *                                                                         *
...
__title__ = "StairsPlus object"
__author__ = "Yorik van Havre, Andres"
__url__ = "https://www.freecad.org"

## @package stairsplus_object
#  \brief The StairsPlus object (a modifiable copy of Arch Stairs)
#
#  This module provides the _StairsPlus parametric object and a
#  makeStairsPlus() factory. It is a renamed copy of FreeCAD's ArchStairs
#  so the geometry engine can be freely extended (curved flights, winders,
#  custom tread profiles, ...) without affecting the built-in Stairs tool.
...
class _StairsPlus(ArchComponent.Component):
    ...
    def setProperties(self, obj):
        ...
        if not hasattr(obj, "LandingStep"):
            obj.addProperty(
                "App::PropertyInteger",
                "LandingStep",
                "Steps",
                QT_TRANSLATE_NOOP(
                    "App::Property",
                    "The step number the landing/turn sits on (0 = auto, centered).",
                ),
                locked=True,
            )

        if not hasattr(obj, "WinderSteps"):
            obj.addProperty(
                "App::PropertyInteger",
                "WinderSteps",
                "Steps",
                QT_TRANSLATE_NOOP(
                    "App::Property",
                    "Number of winder (wedge) steps that sweep the half-turn "
                    "when there is no landing (minimum 2).",
                ),
                locked=True,
            )
    ...
    def execute(self, obj):
        ...
        if obj.RailingLeft:
            ...
            if obj.OutlineLeftAll:
                railWireL, NU = _StairsPlus.returnOutlineWireFace(
        ...
        if obj.RailingRight:
            ...
            if obj.OutlineRightAll:
                railWireR, NU = _StairsPlus.returnOutlineWireFace(
    ...
    def makeMultiEdgesLanding(self, obj, edges):
        ...
        stepWire, stepFace = _StairsPlus.returnOutlineWireFace(
    ...
    def returnOutlines(
        ...
        for i, edge in enum_edges:
            ...
            if align == "Left":
                vBase[i] = _StairsPlus.align(vBase[i], "Right", -vOffsetH)
                if isArc:
                    vBase1 = _StairsPlus.align(vBase1, "Right", -vOffsetH1)
                    vBase2 = _StairsPlus.align(vBase2, "Right", -vOffsetH2)
            elif align == "Right":
                vBase[i] = _StairsPlus.align(vBase[i], "Right", vOffsetH)
                if isArc:
                    vBase1 = _StairsPlus.align(vBase1, "Right", vOffsetH1)
            ...
            p1.append(
                _StairsPlus.align(vBase[i], align, vWidth[i]).add(
            ...
            if isLine:
                ...
                p2.append(
                    _StairsPlus.align(vBase1, align, vWidth1)
                ...
                pArc1.append(
                    _StairsPlus.align(vBase2, align, vWidth2)
                    .add(Vector(0, 0, -abs(treadThickness.Value)))
                    .add(Vector(0, 0, -railStartRiser.Value))
                )
                pArc2.append(
            ...
            if i > 0:
                ...
                intersectionP1P2, intersectionP3P4 = _StairsPlus.findLineArcIntersections(
                    p1last,
                    p2last,
                    p3last,
                    p4last,
    ...
    def makeStraightStairs(
        ...
        if structure == "Massive":
            # numOfSteps < 2 => the step loop is empty, so lProfile would stay
            # empty and lProfile[-1] below would raise IndexError. A 1-step
            # flight has no massive structure to build, so skip it.
            if structureThickness and numOfSteps >= 2:
            ...
            if stringerWidth and structureThickness and numOfSteps >= 2:
    ...
    def makeWinderStairs(self, obj, pivot, uDir, tDir, sideSign, numWinders,
        ...
        The walking direction rotates from +uDir (incoming flight) to -uDir,
        climbing one riser (`hstep`) per winder, and ALWAYS bulges forward
        ...
        def radialDir(theta):
            # +sideSign*tDir at theta=0 (toward the stair body), +uDir at
            # theta=pi/2 (forward bulge), -sideSign*tDir at theta=pi.
            a = Vector(uDir).multiply(math.sin(theta))
            b = Vector(tDir).multiply(sideSign * math.cos(theta))
            return a.add(b)

        def squarePt(z, d, half):
            # Project direction d onto a square boundary of half-size `half`
            # (centred at the pivot), so radials reach the square's corners.
            comp = max(abs(d.dot(uDir)), abs(d.dot(tDir)), 1e-9)
            return Vector(pivot.x, pivot.y, z).add(Vector(d).multiply(half / comp))

        # The square's corners lie on the 45 deg / 135 deg diagonals. If a
        # corner angle falls inside a wedge's span we must add that corner
        ...
        for i in range(numWinders):
            ...
            # First winder sits one riser above flight 1's top tread; each
            # subsequent winder climbs one more.
            ...
            if nosing > 1e-9 and tang.Length > 1e-9:
                ...
                treadPts = [apex.add(noseVec), outer[0].add(noseVec)] \
                    + outer + [apex, apex.add(noseVec)]
            else:
                treadPts = [apex] + outer + [apex]

            tread = Part.Face(Part.makePolygon(treadPts))
            if treadThickness:
                self.steps.append(tread.extrude(Vector(0, 0, -abs(treadThickness))))
            else:
    ...
    def makeWinderTurn(self, obj, edgeP1p2, p1, p2, vLength, vWidth,
        ...
        the rest. v1 - tune pivot / align by eye."""
        ...
        if fl in ("HalfTurnRight", "QuarterTurnRight"):
            sideSign = -1.0
            pivot = p2.add(vWidth)        # far rail
        else:  # left turn (default)
            sideSign = 1.0
        ...
        vWidth2 = DraftVecUtils.scaleTo(wDir, width)
    ...
    def makeStraightStairsWithLanding(
        ...
        # A half-turn and/or a landing both require the flight to be SPLIT at a
        ...
        quarterTurn = ["QuarterTurnLeft", "QuarterTurnRight"]
        fl = flight if flight is not None else obj.Flight
        isHalfTurn = fl in halfTurn
        isQuarterTurn = fl in quarterTurn
        isTurn = isHalfTurn or isQuarterTurn
        totalAngle = (math.pi / 2.0) if isQuarterTurn else math.pi
        ...
        if wantSplit and numOfSteps > 3:
            ...
            # one step on each flight, i.e. 1 <= LandingStep <= numOfSteps - 1.
            ls = int(getattr(obj, "LandingStep", 0) or 0)
            if 0 < ls < numOfSteps:
                landingStep = ls
            else:
            ...
            hasSplit = False
            hasLanding = False
            if wantSplit:  # but not numOfSteps > 3
                print("Fewer than 4 steps, unable to create landing/turn")
        ...
        if treadDepthEnforce and treadDepthEnforce != "Auto":
            ...
            if hasSplit:
                ...
                # check landingDepth
                elif landingDepth and landingDepth != "Auto":  # float
        ...
        if useTurn:
            self.makeWinderTurn(
                ...
                landingStep, numOfSteps, align, flight, totalAngle,
            )
        elif hasSplit:
            ...
            if isHalfTurn:
                ...
                if (align == "Left" and halfTurnLR == "HalfTurnLeft") or (
                    ...
                    p3r = self.align(
                        p2, "Right", -2 * vWidth
                    )  # -ve / opposite direction of "Right" - no "Left" in _StairsPlus.Align()
...
class _ViewProviderStairsPlus(ArchComponent.ViewProviderComponent):
...
# ---------------------------------------------------------------------------
# Factory - creates a _StairsPlus object. Mirrors the single-object path of
# Arch.makeStairs() but instantiates our own (modifiable) class.
# ---------------------------------------------------------------------------
def makeStairsPlus(baseobj=None, length=None, width=None, height=None,
                   steps=None, name="StairsPlus"):
    """Create and return a _StairsPlus object with sensible defaults."""
    ...
    _StairsPlus(obj)
    if FreeCAD.GuiUp:
        _ViewProviderStairsPlus(obj.ViewObject)

    if baseobj:
        obj.Base = baseobj

    # defaults (mirroring Arch.makeStairs setProperty)
