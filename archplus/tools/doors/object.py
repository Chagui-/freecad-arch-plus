...
# *   the DoorsPlus add-on. As a derivative of LGPL-2.1-or-later code it    *
...
__title__ = "DoorsPlus object"
__author__ = "Yorik van Havre, Andres"
__url__ = "https://www.freecad.org"

## @package archplus.tools.doors.object
#  \brief The DoorsPlus object (a modifiable copy of Arch Window)
#
#  This module is a renamed copy of FreeCAD's ArchWindow so the door/window
...
class _ViewProviderWindow(ArchComponent.ViewProviderComponent):
    ...
    def _openDoorsPlusPanel(self, obj):
        "Open the DoorsPlus configuration panel for `obj`. Returns True if shown."
    ...
    def doubleClicked(self, vobj):
        "Double-click a door in the tree -> DoorsPlus panel."

        return self._openDoorsPlusPanel(vobj.Object)

    def setEdit(self, vobj, mode=0):
        "Route the native 'Edit' entry to the DoorsPlus panel too."

        if mode != 0:
            return None
        # Show our panel, then return False so FreeCAD does NOT enter its own
        # edit mode: the panel owns its dialog (and its Edit-Door transaction),
        ...
        self._openDoorsPlusPanel(vobj.Object)
        return False
    ...
    def setupContextMenu(self, vobj, menu):
        ...
        QtCore.QObject.connect(
            actionReposition, QtCore.SIGNAL("triggered()"), self.repositionDoor)
...
# Factory
# ---------------------------------------------------------------------------
def makeWindow(baseobj=None, width=None, height=None, parts=None, name="Door"):
    """Create a _Window object (a DoorsPlus door) in the active document.

    Parameters
    ----------
    baseobj : FreeCAD.DocumentObject, optional
        A sketch whose wires define the door geometry.
