...
# *                                                                         *
# *   This file is a modified copy of FreeCAD's ArchWindow.py, adapted for  *
# *   the WindowsPlus add-on. As a derivative of LGPL-2.1-or-later code it  *
...
__title__ = "WindowsPlus object"
__author__ = "Yorik van Havre, Andres"
__url__ = "https://www.freecad.org"

## @package archplus.tools.windows.object
...
#  geometry engine can be freely extended (better positioning, custom UI, ...)
#  without affecting the built-in Door/Window tools. Windows and doors are
#  Arch objects obtained by extruding a series of wires, and that can be
#  inserted into other Arch objects by defining a volume that gets subtracted
#  from them. Doors are simply Window presets ("Simple door", "Glass door").
...
class _Window(ArchComponent.Component):
    ...
    def buildShapes(self, obj):
        ...
        # glass panels rely on to travel with their sash/leaf. A part that
                ...
                if hinge and omode:
                    ...
                    if p:
                        ...
                        if omode == 1:  # Arc 90
                            ...
                            # Slide the leaf along its own width (chord is the
                            # horizontal span from the hinge edge across the
                            # leaf). Opening = fraction of the leaf width.
                            if opening:
                                slidevec = DraftVecUtils.scaleTo(
                                    chord, chord.Length * opening)
                        elif omode == 10:  # sliding (opposite direction)
                            if opening:
                                slidevec = DraftVecUtils.scaleTo(
                                    chord, -chord.Length * opening)
                exv = FreeCAD.Vector()
...
class _ViewProviderWindow(ArchComponent.ViewProviderComponent):
    ...
    def _openWindowsPlusPanel(self, obj):
        "Open the WindowsPlus configuration panel for `obj`. Returns True if shown."
        ...
        try:
            ...
            FreeCAD.Console.PrintError("WindowsPlus: %s\n" % exc)
            return False

    def doubleClicked(self, vobj):
        "Double-click a window in the tree -> WindowsPlus panel."

        return self._openWindowsPlusPanel(vobj.Object)

    def setEdit(self, vobj, mode=0):
        "Route the native 'Edit' entry to the WindowsPlus panel too."

        if mode != 0:
            return None
        # Show our panel, then return False so FreeCAD does NOT enter its own
        # edit mode: the panel owns its dialog (and its Edit-Window transaction),
        # so there is no setEdit/unsetEdit handshake to keep in sync. This
        # replaces the native _ArchWindowTaskPanel, whose hinge-side / opening-
        # direction controls are already covered by the panel's Swing group.
        self._openWindowsPlusPanel(vobj.Object)
        return False
    ...
    def setupContextMenu(self, vobj, menu):
        ...
        actionReposition = QtGui.QAction(
            translate("Arch", "Reposition (pick point)"), menu)
        QtCore.QObject.connect(
            actionReposition, QtCore.SIGNAL("triggered()"), self.repositionWindow)
        menu.addAction(actionReposition)
    ...
    def repositionWindow(self):
        "Move this window to a new mouse-picked location (WindowsPlus)."
    ...
    def invertHinge(self):
        ...
        self.invertPairs(pairs)
        # Also invert opening direction, so the sash still opens towards
...
def makeWindow(baseobj=None, width=None, height=None, parts=None, name="Window"):
    """Create a _Window object (a WindowsPlus window) in the active document.

    Parameters
    ----------
    baseobj : FreeCAD.DocumentObject, optional
        A sketch whose wires define the window geometry.
    width : float, optional
        Overall width (mm). Written to obj.Width.
    height : float, optional
        Overall height (mm). Written to obj.Height.
    parts : list[str], optional
        Flat list of WindowParts (groups of 5).
    name : str
        Object label.
    ...
    if baseobj is not None and FreeCAD.GuiUp:
        try:
            baseobj.ViewObject.DisplayMode = "Wireframe"
